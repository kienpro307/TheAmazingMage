...
void ThreatsObject::Show(SDL_Renderer* des)
{
	if (come_back_time_ == 0)
	{
		...
		else if (input_type_.left_ == 0 && input_type_.right_ == 1)
		...
		SDL_RenderCopyEx(des, p_object_, currentClip, &rendQuad, NULL, NULL, flipType);
	}
}
...
void ThreatsObject::MakeBullet(SDL_Renderer* screen, const int& x_limit, const int& y_limit)
{
	for (int i = 0; i < bullet_list_.size(); i++)
	{
		...
		if (p_bullet != NULL)
		{
			...
			{
				p_bullet->set_is_move(true);
				p_bullet->SetRect(x_pos_ + 5, y_pos_ + 10);
			}
		}
	}
}